...
fact {
  ...
  some pay: Payment | pay.metodePembayaran != none and pay.jumlah >= 0 and pay.idPembayaran != none and pay.service in PaymentService
  some ew: EWallet | ew.PhoneNumber != none and ew.metodePembayaran != none and ew.jumlah >= 0 and ew.idPembayaran != none and ew.service in PaymentService
  some cc: CreditCard | cc.CardNumber != none and cc.ExpiredDate != none and cc.metodePembayaran != none and cc.jumlah >= 0 and cc.idPembayaran != none and cc.service in PaymentService
  some cod: CashOnDelivery | cod.Confirm != none and cod.metodePembayaran != none and cod.jumlah >= 0 and cod.idPembayaran != none and cod.service in PaymentService
...
assert PaymentAmountMatchesOrder {
  all p: Payment, o: Order | p.service.order = o => p.jumlah = o.harga
}

/*
check PaymentMatchesOrder {
  all o: Order | some p: Payment | p.service.order = o and p.jumlah = o.harga
}

check NoConcurrentOrders {
  no o1, o2: Order | o1.status = Progress and o2.status = Selesai
}

check UniquePaymentIDs {
  all disj p1, p2: Payment | p1.idPembayaran != p2.idPembayaran
}

check ValidPaymentMethods {
  all p: Payment | p.metodePembayaran != none
}
*/
